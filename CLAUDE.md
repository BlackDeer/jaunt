...
6. **Validate:** `node -e "JSON.parse(require('fs').readFileSync('data/<slug>.json'))"` (or `python3 -m json.tool`). Confirm the slug matches the filename.
7. **Tell the user the local + live URLs**, list assumptions made and any images you'd like them to replace, then offer to commit/push.
